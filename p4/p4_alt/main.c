...
static void mk_object(char *key, char *val, uint64_t key_len, uint64_t val_len,
                      uint64_t *key_len_, uint64_t *val_len_, uint64_t i,
                      char code) {
...
static int read_write_restore(const uint64_t N, const uint64_t K,
                              const uint64_t V) {
  ...
  for (i = 0; i < N; ++i) {
    ...
    if (kvdb_insert(kvdb, key, key_len, val, val_len) ||
        ...
        (val_len != val_len_) || memcmp(val, val_, val_len_) ||
        ((i + 1) != kvdb_size(kvdb)) || (0 != kvdb_waste(kvdb))) {
      kvdb_close(kvdb);
      FREE(key);
      FREE(val);
      FREE(val_);
  ...
  for (i = 0; i < N; ++i) {
    ...
    if (kvdb_lookup(kvdb, key, key_len, val_, &val_len_) ||
        (val_len != val_len_) || memcmp(val, val_, val_len_) ||
        (N != kvdb_size(kvdb)) || (0 != kvdb_waste(kvdb))) {
      kvdb_close(kvdb);
      FREE(key);
      FREE(val);
      FREE(val_);
...
static int read_write(const uint64_t N, const uint64_t K, const uint64_t V) {
  uint64_t i, j, key_len, val_len, val_len_;
  void *key, *val, *val_;
  struct kvdb *kvdb;
...
static int read_write_large_restore(void) {
  return read_write_restore(1234, 1234, 1234);
}
static int read_write_small_restore(void) {
  return read_write_restore(1234, 123, 123);
}
static int read_write_single_restore(void) {
  return read_write_restore(1, 23, 23);
}

static int heavy_rewrite(void) {
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (kvdb_update(kvdb, KEY, SLEN(KEY), val, SLEN(val)) ||
        ...
        (SLEN(val) != val_len_) || memcmp(val, val_, val_len_) ||
        (1 != kvdb_size(kvdb)) || (i != kvdb_waste(kvdb))) {
      kvdb_close(kvdb);
      TRACE("software");
      return -1;
    }
...
static int basic_logic(void) {
  ...
  if ((+1 != kvdb_lookup(kvdb, KEY, SLEN(KEY), 0, 0)) ||
      ...
      (0 != kvdb_size(kvdb)) || (0 != kvdb_waste(kvdb))) {
    kvdb_close(kvdb);
    TRACE("software");
    return -1;
  }
  ...
  if (kvdb_insert(kvdb, KEY, SLEN(KEY), VAL1, SLEN(VAL1)) ||
      ...
      (SLEN(VAL1) != val_len) || memcmp(VAL1, val, val_len) ||
      (+1 != kvdb_insert(kvdb, KEY, SLEN(KEY), VAL1, SLEN(VAL1))) ||
      kvdb_lookup(kvdb, KEY, SLEN(KEY), val, &val_len) ||
      (SLEN(VAL1) != val_len) || memcmp(VAL1, val, val_len) ||
      (1 != kvdb_size(kvdb)) || (0 != kvdb_waste(kvdb))) {
    kvdb_close(kvdb);
    TRACE("software");
    return -1;
  }
  ...
  if (kvdb_replace(kvdb, KEY, SLEN(KEY), VAL2, SLEN(VAL2)) ||
      kvdb_lookup(kvdb, KEY, SLEN(KEY), val, &val_len) ||
      (SLEN(VAL2) != val_len) || memcmp(VAL2, val, val_len) ||
      (+1 != kvdb_insert(kvdb, KEY, SLEN(KEY), VAL2, SLEN(VAL2))) ||
      kvdb_lookup(kvdb, KEY, SLEN(KEY), val, &val_len) ||
      (SLEN(VAL2) != val_len) || memcmp(VAL2, val, val_len) ||
      (1 != kvdb_size(kvdb)) || (1 != kvdb_waste(kvdb))) {
    kvdb_close(kvdb);
    TRACE("software");
    return -1;
  }
  ...
  if (kvdb_update(kvdb, KEY, SLEN(KEY), VAL3, SLEN(VAL3)) ||
      kvdb_lookup(kvdb, KEY, SLEN(KEY), val, &val_len) ||
      (SLEN(VAL3) != val_len) || memcmp(VAL3, val, val_len) ||
      (+1 != kvdb_insert(kvdb, KEY, SLEN(KEY), VAL3, SLEN(VAL3))) ||
      kvdb_lookup(kvdb, KEY, SLEN(KEY), val, &val_len) ||
      (SLEN(VAL3) != val_len) || memcmp(VAL3, val, val_len) ||
      (1 != kvdb_size(kvdb)) || (2 != kvdb_waste(kvdb))) {
    kvdb_close(kvdb);
    TRACE("software");
    return -1;
  }
  ...
  if (kvdb_remove(kvdb, KEY, SLEN(KEY), val, &val_len) ||
      (SLEN(VAL3) != val_len) || memcmp(VAL3, val, val_len) ||
      (+1 != kvdb_lookup(kvdb, KEY, SLEN(KEY), val, &val_len)) ||
      (0 != kvdb_size(kvdb)) || (3 != kvdb_waste(kvdb))) {
    kvdb_close(kvdb);
    TRACE("software");
    return -1;
  }
...
int main(int argc, char *argv[]) {
  ...
  TEST(read_write_single_restore, "read_write_single_restore");
  TEST(read_write_small_restore, "read_write_small_restore");
  TEST(read_write_large_restore, "read_write_large_restore");
