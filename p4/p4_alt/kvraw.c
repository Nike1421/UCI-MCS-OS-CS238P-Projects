...
static int read_meta(struct kvraw *kvraw, uint64_t off, struct meta *meta)
{
  ...
  if (('K' != meta->mark[0]) || ('V' != meta->mark[1]) ||
...
int kvraw_lookup(struct kvraw *kvraw, void *key, uint64_t *key_len, /* in/out */
                 void *val, uint64_t *val_len,                      /* in/out */
                 uint64_t *off)                                     /* in/out */
{
...
int kvraw_append(struct kvraw *kvraw, const void *key, uint64_t key_len,
                 const void *val, uint64_t val_len, uint64_t *off)
{
  struct meta meta;
  uint64_t off_;
