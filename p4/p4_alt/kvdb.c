...
static int rebuild_index(struct kvdb *kvdb, int key)
{
  uint64_t off = 0;
  struct meta meta;
  int result;

  /* Iterate over all entries in kvraw */
  while (off < kvdb->kvraw->size)
  {
    /* Read the metadata at the current offset */
    if (read_meta(kvdb->kvraw, off, &meta) != 0)
    {
      return -1;
    }

    /* Read the key at the current offset */
    if (logfs_read(kvdb->kvraw->logfs, key, KEY_OFF(off), meta.key_len) != 0)
    {
      return -1;
    }

    /* Update the index with the key and the current offset */
    uint64_t *ref = index_update(kvdb->index, key, meta.key_len);
    if (ref == NULL)
    {
      return -1;
    }
    *ref = off;
    ...
    off += META_LEN + meta.key_len + meta.val_len;
  }
